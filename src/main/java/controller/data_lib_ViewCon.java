...
@WebServlet("/data_lib_view.do")
public class data_lib_ViewCon extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		data_lib_DTO dto = dao.selectView(num); // 게시물 세부정보 보기 호출
		dao.close();
		
		// 게시물 dto 객체 저장 후 뷰 파일(게시물 세부정보)로 포워드 시킴
		request.setAttribute("dto", dto);
		request.getRequestDispatcher("/Data_lib_View.jsp").forward(request, response);
	}
